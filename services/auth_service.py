"""
Deployr — Auth Service

Framework-agnostic service implementing the OAuth 2.0 Authorization Code
flow with PKCE against meta.wikimedia.org, plus stateless JWT token
management.

This module never imports Flask or any web framework — the web layer
(routes/auth.py) calls into this service.
...
def build_authorize_url(client_id, redirect_uri, code_challenge, state):
    ...
    The web layer should redirect the user's browser to this URL.
...
def fetch_user_profile(access_token):
    ...
    try:
        ...
        raise OAuthError(f"Profile request failed (HTTP {e.code}): {body}") from e
...
def _post_token_request(data):
    ...
    try:
        ...
        body = e.read().decode("utf-8", errors="replace")
        raise OAuthError(f"Token request failed (HTTP {e.code}): {body}") from e
    except urllib.error.URLError as e:
